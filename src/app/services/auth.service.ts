...
import { ApplicantWiseJob, ChangeJobStatus, PostedJob, PostJob } from '../Interface/employerModel';
...
//=====================demo Varuiable to store candidate data========================
  ...
  getApplicantWiseJobs(employerId: number): Observable<ApplicantWiseJob[]> {
    return this.http.get<ApplicantWiseJob[]>(`${this.employerUrl}/allApplnWisejobs/${employerId}`);
  }

  // getApplicantsForJob(jobId: number): Observable<any> {
  //   return this.http.get<any>(`${this.employerUrl}/allApplicants/${jobId}`);
  // }

  // This is not working just generated by AI
  // getApplicantsForJob(jobId: number, filters: { status?: string | null; search?: string | null }): Observable<any> {
  //   let params = new HttpParams();
  //   if (filters.status) {
  ...
  //   }
  //   return this.http.get<any>(`${this.employerUrl}/allApplicants/${jobId}`, { params });
  // }

  getApplicantsForJob(jobId: number) {
    return this.http.get<any>(`${this.employerUrl}/allAppln/${jobId}`);
  }
  // This is not working just generated by AI

  updateApplicationStatus(applicationId: number, newStatus: string): Observable<any> {
    return this.http.post(`${this.employerUrl}/updateApplicationStatus`, { applicationId, newStatus });
  }
